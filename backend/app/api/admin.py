...
from backend.app.core.dependencies import require_editor, require_admin, get_storage
from backend.app.db.session import get_db
from backend.app.models.user import User
from backend.app.models.publish_run import PublishRun
from backend.app.schemas.catalogue import ValidationReport, PublishRunOut
...
def get_reference_config(_user: User = Depends(require_editor)):
...
def get_publish_history(
    ...
    runs = db.query(PublishRun).order_by(PublishRun.started_at.desc()).offset(skip).limit(limit).all()
    return runs
